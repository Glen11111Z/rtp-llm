...
BeamSearchOutput ROCmDevice::sampleBeamSearch(const BeamSearchParams& params) {
    ...
    RTP_LLM_CHECK_WITH_INFO((vocab_size > 2 * beam_width_in),
                            "rocm beam search op need vocab_size[%d] > beam_width_in[%d] * 2",
                            vocab_size,
                            beam_width_in);
    RTP_LLM_CHECK_WITH_INFO((vocab_size > 2 * beam_width_out),
                            "rocm beam search op need vocab_size[%d] > beam_width_out[%d] * 2",
                            vocab_size,
                            beam_width_out);
        ...
        switch (T_EXPR) {                                                                                              \
            ...
            default:                                                                                                   \
                RTP_LLM_CHECK_WITH_INFO(                                                                               \
                    false, "rocm beam search op does not support dtype[%d]", params.logits.type());                    \
        }                                                                                                              \
    } while (0)
...
#define DISPATCH_BOOL(BOOL, BOOL_EXPR, ...)                                                                            \
    ...
    hipMemsetAsync(workspace->data(), 0, workspace->sizeBytes(), stream_);
